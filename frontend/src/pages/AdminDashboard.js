    ...
    <div className="min-h-screen py-12 bg-background-paper">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex space-x-4 mb-8 overflow-x-auto">
          {["analytics", "insights", "products", "categories", "orders", "shipping", "whatsapp", "support", "banners", "coupons", "gst", "settings"].map((tab) => (
            <button
              key={tab}
              ...
              data-testid={`tab-${tab}`}
            >
              {tab === "gst" ? "GST & Invoice" : tab === "whatsapp" ? "WhatsApp AI" : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </button>
          ))}
        </div>
